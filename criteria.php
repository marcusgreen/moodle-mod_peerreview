...
class peerreview_criteria_form extends moodleform {
    ...
    public function definition() {
        ...
        $repeatarray[] = $mform->createElement('text', 'criterionDescription',
            $description, array('size' => '80', 'class' => 'criteriaField'));
        $review = get_string('citerionatreview', 'peerreview');
        $repeatarray[] = $mform->createElement('text', 'criterionReview',
            $review, array('size' => '80', 'class' => 'criteriaField'));
        $valuestring = get_string('valueofcriterion', 'peerreview');
        $valueparameters = array(
            'size' => '3', 'onkeyup' => 'M.peerreview.updateTotal();',
        ...
        $placeholder = HTML_WRITER::tag('div', 2 * $this->_customdata['reviewreward'],
            array('id' => 'rewardCell', 'class' => 'markSummaryValue'));
        $mform->addElement('static', '', $label, $placeholder);
...
if ($mform->is_cancelled()) {
    ...
    for ($i = 0; $i < $fromform->option_repeats; $i++) {
        if ($fromform->criterionDescription[$i] != '') {
            ...
            if ($existingrecord = $DB->get_record('peerreview_criteria',
                        array('peerreview' => $peerreviewid, 'ordernumber' => $i))) {
    ...
    redirect($CFG->wwwroot . '/mod/peerreview/view.php?id=' . $cm->id . '#criteria',
         get_string('criteriaupdated', 'peerreview'), 3);
} else {
